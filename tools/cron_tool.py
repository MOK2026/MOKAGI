...
def _cron_confirm_ttl(agent_config: dict = None) -> float:
    """cron 確認碼有效期限（秒），0=不限時。
    優先序：環境變數 MOK_ADMIN_CONFIRM_TTL → Agent 設定檔 MOK_ADMIN_CONFIRM_TTL → 預設 300 秒。"""
    raw = os.environ.get("MOK_ADMIN_CONFIRM_TTL", "")
    if raw == "" and agent_config:
        raw = str(agent_config.get("MOK_ADMIN_CONFIRM_TTL", ""))
    if raw in (None, ""):
        return 300.0
    try:
        return float(raw)
...
def _cron_confirm_expired(timestamp: float, ttl: float) -> bool:
...
def _load_cron_pending(ttl: float = None) -> None:
    d = _cron_pending_dir()
    try:
        if ttl is None:
            ttl = _cron_confirm_ttl()
        ...
        for fn in os.listdir(d):
            ...
            try:
                with open(p, "r", encoding="utf-8") as f:
                    item = json.load(f)
                if isinstance(item, dict) and not _cron_confirm_expired(item.get("timestamp", 0), ttl):
...
CRON_TASK_DESCRIPTIONS = {
    ...
    "indexPage": {
        "title": "🖥 indexPage 房間快照 · 每分鐘同步到網頁目錄 html/_x",
        "desc": (
            "每分鐘把 /home/ubuntu/.mok/agent/indexPage 房間內所有檔案複製一份到 "
            "/home/ubuntu/.mok/html/_x/（檔名加 .txt），並輸出 00_list.txt（目錄清單）、"
            "01_files.txt（近期檔案）、02_grep62655.txt（搜尋 62655 的結果）、00_done.txt（完成標記）。"
            "用途：讓主人可經瀏覽器/網頁直接讀取 indexPage 的工作狀態快照。"
        ),
        "view": (
            "  • 網頁目錄：ls /home/ubuntu/.mok/html/_x/\n"
            "  • 完成標記：cat /home/ubuntu/.mok/html/_x/00_done.txt"
        ),
        "delete": "/cron delete（請先 /cron list 查看目前編號；刪除後即停止同步）",
        "risk": "⚠️ 中風險：每分鐘執行、持續寫入 html/_x；僅複製不刪除原檔。",
        "code": "mkdir -p html/_x → cp indexPage/* → 目錄/檔案清單 → grep 62655 → echo DONE",
    },
}
...
_CRON_AT_CN = {"@reboot": "系統開機時", "@daily": "每天", "@hourly": "每小時",
               "@weekly": "每週", "@monthly": "每月", "@yearly": "每年", "@annually": "每年"}


def _humanize_cron_schedule(cron_line):
    """把 cron 排程整行翻譯成中文，讓非程式使用者也能看懂同意的是什麼。"""
    raw = cron_line.strip().strip(chr(34))
    parts = raw.split()
    if not parts:
        return ""
    if parts[0].lower() in _CRON_AT_CN:
        when = _CRON_AT_CN[parts[0].lower()]
        cmd = " ".join(parts[1:])
    elif len(parts) >= 6:
        ...
        if hour == "*" and minute == "*":
            ...
            hm = "%02d:%02d" % (int(hour), int(minute))
        elif hour.isdigit() and minute == "*":
            hm = "%02d:00" % int(hour)
            ...
            hm = "每 %s 分鐘" % minute[2:]
        elif hour.startswith("*/") and minute == "*":
            hm = "每 %s 小時" % hour[2:]
        else:
            hm = "%s 點 %s 分" % (hour, minute)
        when = ""
        if dow != "*" and dow.isdigit():
            when = "每週%s" % _WEEKDAY_CN[int(dow) % 7]
        ...
        if not when:
            if minute == "*" and hour == "*":
                when = "每分鐘"
            elif hm and not hm.startswith("每"):
                when = "每天"
        when = (" ".join([x for x in (when, hm) if x]) or "（自訂時間）")
    else:
        return ""
    return (f"⏰ 執行時間：{when}\n"
            f"🖥️ 執行指令：`{cmd}`\n"
            "⚠️ 這是一個自訂指令，若不確定它的用途，請勿直接確認，可先詢問管理員。")
...
async def confirm_cron_command(chat_id: str, token: str, agent_config: dict = None) -> tuple:
    ...
    _load_cron_pending(ttl=_cron_confirm_ttl(agent_config))
    ...
    if _cron_confirm_expired(info["timestamp"], _cron_confirm_ttl(agent_config)):
        del _pending_cron_confirmations[token]
        _remove_cron_pending_file(token)
        return False, "❌ 確認碼已超時。請重新發送原命令。"
...
async def handle_cron(args, chat_id: str = None, agent_config: dict = None):
    ...
    # ----- add (需確認) -----
        ...
        # 去掉誤包在整串指令外圍的引號（避免寫入 crontab 後整行失效）
        if len(rest) >= 2 and rest.startswith(chr(34)) and rest.endswith(chr(34)):
            rest = rest[1:-1].strip()
        # 產生確認碼
        ...
        if not _desc:
            _desc = _humanize_cron_schedule(rest)
        if _desc:
            warning += f"\n\n📖 任務說明：\n{_desc}"
    ...
    # ----- delete (需確認) -----
        ...
        if not _desc:
            _desc = _humanize_cron_schedule(target_line)
        if _desc:
            warning += f"\n\n📖 任務說明：\n{_desc}"
        return f"CONFIRM_SPLIT:{warning}\n🔐 此確認碼用於授權執行上方操作（僅限您本人確認）。若您未發起此操作，請直接忽略。\n請在 5 分鐘內發送確認碼以執行：\n---CONFIRM_SPLIT---\n/admin confirm {token}"
