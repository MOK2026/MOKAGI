...
#
#方案 B：放寬 /admin confirm 確認碼時限（預設 5 分鐘）
# 在 Agent 設定檔中加入（秒）：MOK_ADMIN_CONFIRM_TTL=0
# 0 = 不設時間限制；也可設 86400（1天）、3600（1小時）等；留空/未設定則維持 300（5 分鐘）
# 亦可透過環境變數 MOK_ADMIN_CONFIRM_TTL 覆寫（優先於設定檔）

# ------------------------------------------------------------------------------------ #
PENDING_CONFIRM_DIR = os.path.join(MOKAGI_HOME, ".pending_admin_confirm")
PENDING_CONFIRM_TTL = 300  # 預設 5 分鐘；可被 MOK_ADMIN_CONFIRM_TTL（環境變數或 Agent 設定檔）覆寫

def _get_confirm_ttl(agent_config: dict = None) -> float:
    """取得確認碼有效期限（秒）。0 或負值 = 不設時間限制。
    ...
    if raw in (None, ""):
        return float(PENDING_CONFIRM_TTL)
    try:
        return float(raw)
...
def _confirm_ttl_hint(ttl: float) -> str:
    ...
    if ttl <= 0:
        return "不限時間"
    if ttl >= 86400:
        return f"{ttl / 86400:g} 天"
    if ttl >= 3600:
        return f"{ttl / 3600:g} 小時"
    if ttl >= 60:
        return f"{ttl / 60:g} 分鐘"
    return f"{int(ttl)} 秒"
...
async def confirm_command(chat_id: str, token: str, agent_config: dict = None) -> tuple:
    ...
    try:
        disk_data = _load_pending_confirmations(ttl=_get_confirm_ttl(agent_config))
    ...
    if _confirm_expired(info["timestamp"], _get_confirm_ttl(agent_config)):
    ...
    if cmd_type == "ollama_rm":
        ...
        success, result = execute_shell_command(args, agent_config=agent_config)
    ...
    if args.startswith("exec"):
        ...
        if risk == 'safe':
            if use_docker:
                ...
                success, result = execute_shell_command(rest, agent_config=agent_config)
            return result if success else f"❌ 執行失敗: {result}"
