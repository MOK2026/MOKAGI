...
PLUGIN_INFO = {
    ...
    "tool_schema": {
        ...
        "description": (
            ...
            "並依當前頻道原路回覆：Telegram 用語音訊息，網頁版直接在頁面播放語音。\n\n"
            ...
            "- cantonese/gaam：粵語女聲\n\n"
            "【注意】\n"
            "- 自動判斷頻道：Telegram 用語音訊息回覆；網頁版回傳可播放網址、直接在頁面播放\n"
...
async def handle_tts(args, chat_id="web", agent_config=None):
    ...
    # --- Web 版：原路回覆，直接在網頁播放（不經 Telegram）---
    _cid_raw = str(chat_id or "").strip()
    if _cid_raw.startswith("web"):
        try:
            import uuid as _uuid
            _base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            _tts_dir = os.path.join(_base, "html", "vtuber", "tts")
            os.makedirs(_tts_dir, exist_ok=True)
            _fname = "tts_" + _uuid.uuid4().hex[:10] + ".mp3"
            _fpath = os.path.join(_tts_dir, _fname)
            _web_voice = _resolve_voice(voice_name)
            _com = edge_tts.Communicate(text, _web_voice)
            await _com.save(_fpath)
            if os.path.exists(_fpath) and os.path.getsize(_fpath) > 100:
                return json.dumps({
                    "success": True,
                    "web": True,
                    "url": "/vtuber/tts/" + _fname,
                    "voice": _web_voice,
                    "text_length": len(text),
                    "message": "語音已生成，將於網頁直接播放"
                }, ensure_ascii=False)
        except Exception as _e:
            logger.error(f"[TTS] Web 語音生成失敗: {_e}", exc_info=True)
