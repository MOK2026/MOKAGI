...
// ============ CONSTANTS & STATE ============
const AGENTS_DATA = window.AGENTS_DATA && window.AGENTS_DATA.length ? window.AGENTS_DATA : [{name:"莫氏集團",icon:"💰",post:"",color:0xffffff}]; AGENTS_DATA.forEach(a=>{a.atkInterval=a.atkInterval??1.0;a.atkSpeed=a.atkSpeed??0.3;a.counterRate=a.counterRate??0.25;a.normalDmg=a.normalDmg??2;a.blockDmg=a.blockDmg??1;a.counterDmg=a.counterDmg??3;a.hp=a.hp??10;a.battleTime=a.battleTime??5;}); const AGENTS = window.AGENTS && window.AGENTS.length ? window.AGENTS : AGENTS_DATA.map(a=>a.name);
...
function sendChatMessage(msg){
  ...
  if(socket&&socket.connected){
    ...
    setTimeout(()=>{
      chatLoading.classList.remove('show');
      const replies = [
        '你好！我是 '+state.currentInteractAgent+'，有什麼可以幫你的？',
        '這裡是莫氏 AI 村莊，歡迎來到格鬥競技場！',
        '主人說要在這裡好好招待每位訪客～',
        '想要來一場格鬥對戰嗎？點擊「格鬥」按鈕挑戰我吧！'
      ];
      addChatMessage('agent',replies[Math.floor(Math.random()*replies.length)]);
...
function startBattle(agentName){
  ...
  state.playerHP=state.battleHP;state.enemyHP=state.battleHP;
  ...
  state.battleInterval = setInterval(()=>{
    ...
    if(state.battleTimer<=0){endBattle('timeout');}
    // Enemy AI
    // 🔥 每 atkInterval 秒，以 atkSpeed 機率觸發敵人攻擊
    if(Math.random()<state.battleAtkSpeed && state.battleActive){
      ...
      if(enemyMove==="block"){battleLog.textContent = "🛡 "+state.currentInteractAgent+" 進入格擋！";}else{
      state.enemyLastMoveTime = Date.now();
      battleLog.textContent = "👊 "+state.currentInteractAgent+" 使出 "+moveLabel(enemyMove)+"！快反擊！";}
    }
    drawBattleArena();
...
function playerMove(move){
  ...
  if(state.enemyLastMove && (now-state.enemyLastMoveTime)<1200){
    ...
    if(counterMap[move]===state.enemyLastMove){
      // Counter success!
      state.enemyHP -= state.battleCounterDmg;
      battleLog.textContent='⚡ 反擊成功！'+moveLabel(move)+' 剋制 '+moveLabel(state.enemyLastMove)+'！(-'+state.battleCounterDmg+')';
      flashBattle('counter');
    }else{
      // Normal hit
      if(state.enemyLastMove==='block'){state.enemyHP-=state.battleBlockDmg;battleLog.textContent='💥 擊中格擋！(-'+state.battleBlockDmg+')';}
      else{state.enemyHP-=state.battleNormalDmg;battleLog.textContent='💥 '+moveLabel(move)+'！(-'+state.battleNormalDmg+')';}
    }
    state.enemyLastMove=null;
  }else{
    state.enemyHP -= state.battleNormalDmg;
    battleLog.textContent = '💥 '+moveLabel(move)+'！(-'+state.battleNormalDmg+')';
  }
  ...
  setTimeout(()=>{
    ...
    if(Math.random()<state.battleCounterRate){
      ...
      const counterMap={up:'down',down:'up',left:'right',right:'left'};
      if(state.blockHeld){state.playerHP-=state.battleBlockDmg;battleLog.textContent="🛡 格擋敵方攻擊！(-'+state.battleBlockDmg+')";}else if(counterMap[em]===move){state.playerHP-=state.battleCounterDmg;battleLog.textContent="💢 被反擊！(-'+state.battleCounterDmg+')";}
      else{state.playerHP-=state.battleNormalDmg;battleLog.textContent="💢 "+state.currentInteractAgent+" "+moveLabel(em)+"！(-'+state.battleNormalDmg+')";}
...
function endBattle(result){
  ...
  if(result==='win'){
    state.score.wins++;
    battleResult.className='win';battleResult.textContent='🏆 勝利！';
    battleLog.textContent='🎉 你擊敗了 '+state.currentInteractAgent+'！';
  }else if(result==='lose'){
    state.score.losses++;
