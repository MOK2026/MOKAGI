...
let currentHtmlContent = '';      // 儲存當前 HTML 檔案的完整內容
    ...
    const Mok_Token_Price = 0.000006
...
function renderChatMessages(messages) {
    ...
    messages.forEach(msg => {
        ...
        if (msg.role === 'user') {
            ...
            const estimatedCost = (estimatedTokens * 0.000006).toFixed(6);
            // -------------------------------
            // 用戶消息
            ...
            div.innerHTML = `
                ...
                <div class="message-meta">[#${seqNum}] <button class="quote-id-btn" data-conv-id="${msg.conv_id || '?'}" style="background:#333; color:#4ec9b0; border:1px solid #4ec9b0; border-radius:4px; cursor:pointer; font-size:inherit; padding:0 4px;" title="複製引用對話ID">[ID:${msg.conv_id || '?'}]</button> ${time} · 🧮 ~${estimatedTokens} tokens · 💰 ~¥${estimatedCost}</div>
            `;
            ...
            div.innerHTML = `
                ...
                <div class="message-meta">[#${seqNum}] <button class="quote-id-btn" data-conv-id="${msg.conv_id || '?'}" style="background:#333; color:#4ec9b0; border:1px solid #4ec9b0; border-radius:4px; cursor:pointer; font-size:inherit; padding:0 4px;" title="複製引用對話ID">[ID:${msg.conv_id || '?'}]</button> ${currentAgent} · ${time} · 🧮 ~${estimatedTokens} tokens · 💰 ~¥${estimatedCost}</div>
            `;
            chatMessagesDiv.appendChild(div);
        }
    });
...
async function addUserMessageAndSave(content) {
    ...
    // 使用 V4 Pro 輸出價格（6元/百萬tokens），即 0.000006 元/token
    const estimatedCost = (estimatedTokens * 0.000006).toFixed(6);
    // -------------------------------
    div.innerHTML = `
        ...
        <div class="message-meta">輸出已捷斷 ${new Date().toLocaleString()} · 🧮 ~${estimatedTokens} tokens · 💰 ~¥${estimatedCost}</div>
    `;
...
async function addAssistantMessageAndSave(thinkContent, replyContent) {
    ...
    div.innerHTML = thinkHtml + `
        ...
        <div class="message-meta">${currentAgent} · ${new Date().toLocaleString()} · 🧮 ~${estimatedTokens} tokens · 💰 ~¥${estimatedCost}</div>
    `;
    const copyBtn = div.querySelector('.copy-msg-btn');
    copyBtn.addEventListener('click', copyMsgHandler);
    chatMessagesDiv.appendChild(div);
...
async function loadFile(path, name, silent = false) {
    ...
    if (ext === 'html' || ext === 'htm') {
        ...
        newPreviewBtn.addEventListener('click', function() {
            ...
            const editorContent = document.getElementById('file-editor').value;
        ...
        newEditBtn.addEventListener('click', function() {
            ...
            // 確保編輯器內容為最新
            fileEditor.value = currentHtmlContent;
        });
...
function loadEditorState() {
    ...
    if (state && state.path) {
        ...
        // 若內容有變，填入編輯器
        const editor = document.getElementById('file-editor');
        if (editor && state.content !== undefined) {
    ...
    try {
        ...
        let html = `
            <div style="padding: 12px;">
                <h3>📊 Token 用量統計</h3>
                <div style="background: #2d2d30; padding: 12px; border-radius: 8px; margin-bottom: 16px;">
                    <strong>總用量</strong><br>
                    <span style="font-size: 28px; font-weight: bold; color: #4ec9b0;">${formatNum(data.total_tokens)}</span> tokens
                </div>
        `;

        // 按 Agent 統計
        if (data.by_agent && data.by_agent.length) {
            html += `<h4>🤖 按 Agent</h4><ul style="list-style: none; padding-left: 0;">`;
            for (const item of data.by_agent) {
                html += `<li><strong>${escapeHtml(item.agent_name)}</strong>: ${formatNum(item.tokens)} tokens</li>`;
            }
            html += `</ul>`;
        }
        ...
        if (data.by_model && data.by_model.length) {
            html += `<h4>🧠 按模型</h4><ul style="list-style: none; padding-left: 0;">`;
            for (const item of data.by_model) {
                html += `<li><strong>${escapeHtml(item.model_name)}</strong>: ${formatNum(item.tokens)} tokens</li>`;
            }
            html += `</ul>`;
        }
        ...
        if (data.conversations && data.conversations.length) {
            html += `<h4>💬 最近對話（按會話）</h4><ul style="list-style: none; padding-left: 0;">`;
            for (const conv of data.conversations) {
                const time = new Date(conv.time * 1000).toLocaleString();
                html += `<li><strong>${escapeHtml(conv.agent)}</strong>: ${formatNum(conv.tokens)} tokens<br><span style="font-size: 0.75rem; color: #aaa;">${time}</span></li>`;
            }
            html += `</ul>`;
        }
        ...
        if (data.recent && data.recent.length) {
            html += `<h4>🕒 最近調用明細</h4><div style="max-height: 300px; overflow-y: auto;">`;
            html += `<table style="width:100%; border-collapse: collapse; font-size: 0.8rem;">`;
            html += `<tr><th>時間</th><th>Agent</th><th>模型</th><th>Prompt</th><th>生成</th><th>總計</th></tr>`;
            for (const row of data.recent) {
                const time = new Date(row.timestamp * 1000).toLocaleString();
                html += `<tr style="border-bottom:1px solid #3e3e42;">
                            <td style="padding:4px;">${time}</td>
                            <td>${escapeHtml(row.agent_name)}</td>
                            <td>${escapeHtml(row.model_name || '-')}</td>
                            <td style="text-align:right">${formatNum(row.prompt_tokens)}</td>
                            <td style="text-align:right">${formatNum(row.completion_tokens)}</td>
                            <td style="text-align:right">${formatNum(row.total_tokens)}</td>
                        </tr>`;
            }
            html += `</table></div>`;
        }

        if (!data.by_agent && !data.by_model && !data.conversations && !data.recent) {
            html += `<div>暫無數據，請先進行一些對話。</div>`;
        }
        ...
        console.error(err);
        container.innerHTML = '<div>❌ 加載統計數據失敗</div>';
    }
}
        ...
        <div style="height:100%; display:flex; flex-direction:column;">
            ...
            <div id="content-area" style="height:50%; display:flex; flex-direction:column; overflow:hidden;">
                ...
                <div id="editor-area" style="display:none; flex:1; flex-direction:column; padding:8px;">
                    ...
                    <textarea id="file-editor" style="flex:1; background:#1e1e1e; color:#d4d4d4; border:1px solid #3e3e42; border-radius:8px; padding:8px; resize:none; font-family:monospace;"></textarea>
                </div>
            </div>
