...
def _is_db_lock_error(e):
    """判斷是否為 SQLite 'database is locked' 鎖衝突（暫時性，退避重試即可）。"""
    msg = str(e)
    return ("locked" in msg) and ("database" in msg)
...
async def retry_with_autofix(
    ...
    # ---------- 第 0 階段：SQLite 鎖衝突專用指數退避重試（不觸發 LLM 修復）----------
    # 多 Agent 共用同一 SQLite，寫入衝突是暫時性問題；退避等待即可，不需浪費 LLM autofix。
    lock_retries = 5
    for lock_attempt in range(1, lock_retries + 1):
        try:
            return await _call_handler(action_func, *action_args, **action_kwargs)
        except Exception as e:
            if not _is_db_lock_error(e):
                last_exception = e
                break
            last_exception = e
            wait = 1.0 * (2 ** (lock_attempt - 1))
            logging.warning(f"[retry_with_autofix] SQLite 鎖衝突（第 {lock_attempt}/{lock_retries} 次），{wait}s 後退避重試: {str(e)[:120]}")
...
async def autofix_run(
    ...
    if autofix_max_retries is not None:
        try:
            max_autofix_retries = int(autofix_max_retries)
        except Exception:
            max_autofix_retries = None
    ...
    while attempt < max_attempts:
        ...
        try:
            ...
            if _is_db_lock_error(e):
                if lock_retries_left > 0:
                    lock_retries_left -= 1
                    wait = 1.0 * (2 ** (5 - lock_retries_left))
                    logging.warning(f"[autofix_run] SQLite 鎖衝突（剩餘退避 {lock_retries_left} 次），{wait}s 後重試: {str(e)[:120]}")
                    await asyncio.sleep(wait)
                    attempt -= 1
                    continue
                logging.error(f"[autofix_run] SQLite 鎖衝突退避耗盡，放棄: {str(e)[:120]}")
                break
