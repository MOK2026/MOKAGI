...
def start_desktop():
    """啟動桌面常駐服務（Xvfb + Chromium CDP）"""
    env = os.environ.copy()
    env["MOKAGI_HOME"] = "mok"
    env["PYTHONPATH"] = f"{str(PROJECT_DIR / 'core')}:{str(PROJECT_DIR)}:{str(AGENT_ROOT)}"

    desktop_script = PROJECT_DIR / "core" / "desktop_service.py"
    if not desktop_script.exists():
        log_with_prefix("[Desktop]", f"錯誤: {desktop_script} 不存在")
        return None
...
def main():
    ...
    # 啟動桌面服務（Xvfb + Chromium CDP）- 最先啟動，供後續服務復用
    print("正在啟動桌面服務...", flush=True)
    desktop_proc = start_desktop()
    if desktop_proc:
        processes.append(desktop_proc)
        threading.Thread(target=stream_reader, args=(desktop_proc.stdout, "[Desktop]", output_queue), daemon=True).start()
        if desktop_proc.stderr:
            threading.Thread(target=stream_reader, args=(desktop_proc.stderr, "[Desktop][ERR]", output_queue), daemon=True).start()
    else:
        print("啟動桌面服務失敗（已跳過）", flush=True)
