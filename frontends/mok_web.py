...
# ===== 插話補丁（interject_patch）：工作中輸入框仍可用，訊息併入當前工作輪，不改 mokagi.py =====
try:
    ...
    interject_patch.register_routes(app, is_running_fn=lambda a: a in _running_agents)
except Exception as _ij_e:
    print('[interject_patch] load failed:', _ij_e)
...
def _resolve_user_id(raw=None):
    """解析使用者識別碼（多用戶隔離的關鍵）。

    優先序：請求自帶 user_id → 當前選中 Agent 的 ADMIN_CHAT_ID → 帶隨機後綴的臨時 id。
    不再回退到全域 ADMIN_CHAT_ID / 環境變數的固定值，避免所有匿名訪客共用同一個
    user_key，導致對話歷史互相竄流。
    """
    uid = str(raw or "").strip()
    if uid:
        return uid
    if _current_admin_chat_id:
        cid = str(_current_admin_chat_id).strip()
        if cid:
            return cid
    tmp = "web_guest_" + _uuid.uuid4().hex[:8]
    try:
        print(f"[user_id] 請求未帶 user_id，改用臨時 id：{tmp}")
    except Exception:
        pass
    return tmp
    ...
    def _sse_bg_worker():
        ...
        def _feed_agg(event):
            try:
                ...
                if _et == "iteration_start":
                    ...
                    if event.get("subtype", "normal") not in ("pending_list", "tool_process", "semantic_search", "experience", "interject_note"):
                        if not agg_rounds:
                            agg_rounds.append({"think": "", "tool_calls": [], "tool_results": [], "reply": "", "iteration": 1})
        ...
        def _emit_event(event):
            ...
            if event["type"] == "think":
                ...
                if event.get("subtype", "normal") in ("pending_list", "tool_process", "semantic_search", "experience", "interject_note"):
                    return
                ...
                # 🔧 兜底(跨裝置完整落盤)：整輪 reply 仍空 → 補內容，避免 DB 落盤 content=''
                if not (accumulated_reply or "").strip():
                    _fb = (event.get("content") or "").strip()
                    if _fb:
                        accumulated_reply = _fb
                    elif (accumulated_think or "").strip():
                        accumulated_reply = "（⚠️ 此輪僅有思考過程，文字回答未完整落盤）"
                    else:
                        accumulated_reply = "（⚠️ 此輪無文字回答）"
                if assistant_msg_id is None:
                    with closing(sqlite3.connect(DB_PATH, timeout=30)) as conn:
        ...
        try:
            async def _bg_coro():
                async def async_stream_cb(event):
                    ...
                    if event.get("type") == "done":
                        try:
                            import girl_gate
                            # 用本輪累積回覆（done 事件多數路徑不帶 final_reply，會導致 gate 失效）
            ...
            _schedule_sse_cleanup(session_id, delay_sec=120)
            # ===== 插話補丁：本輪結束仍有未消費的插話 → 自動續開一輪把它做掉 =====
            try:
                # 🔧 20260913：不再另開新對話。插話正常會在本輪工具迴圈內被吸收；
                #    若仍有殘留，保留在佇列，等下一次 LLM 呼叫（下一輪）自動併入。
...
def _latest_conv_id(agent):
    """取該侍女最近一筆對話的 conv_id，讓開機訊息歸到同一串對話。"""
    try:
        with closing(sqlite3.connect(DB_PATH, timeout=30)) as conn:
            row = conn.execute(
...
def _save_girl_maid_reply(agent, content, conv_id=None):
    """把侍女對「開 vast girl」的對話回答落盤到 chat_history。
    ...
    if not content:
        return None
    if conv_id is None:
        conv_id = _latest_conv_id(agent)
    try:
        with closing(sqlite3.connect(DB_PATH, timeout=30)) as conn:
            cur = conn.execute(
                'INSERT INTO chat_history (agent, role, content, think_content, conv_id, timestamp) '
                'VALUES (?, ?, ?, ?, ?, ?)',
                (agent, 'assistant', content, None, conv_id, time.time()))
            conn.commit()
...
def handle_chat_message(data):
    ...
    def _bg_worker():
        ...
        def stream_emit(event):
            ...
            # 🔧 首次發送時附帶 SSE session_id，讓客戶端知道備援通道（仍照常累積並落盤，避免首事件漏寫）
...
def _update_user_message_conv_id(agent, conv_id, user_msg_id, user_id=None):
    ...
    if conv_id is None:
        ...
        conv_id = None
        with closing(sqlite3.connect(hist_db, timeout=30)) as conn:
            # 先用 user_key 查找
            cursor = conn.execute(
...
def handle_stop():
    ...
    # Resilient 版（20260910）：先判斷 mok_agi 是否已被 pm2 註冊／運行
    # - pm2 pid 有輸出（含 0＝已註冊但停止）→ pm2 restart（停止中 restart 會直接喚起）
    # - 完全不存在（pm2 pid 無輸出）→ pm2 start launcher.py --name mok_agi
    try:
        _pid_out = subprocess.run(
            "pm2 pid mok_agi", shell=True, capture_output=True, text=True, timeout=15
        ).stdout.strip()
    except Exception:
        _pid_out = ""
    if _pid_out.isdigit():
        _cmd = "pm2 restart mok_agi"
    else:
        _cmd = "pm2 start /home/ubuntu/.mok/core/launcher.py --name mok_agi"
    # 立即執行（不等待，後臺運行）
    subprocess.Popen(_cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    print(f"立即停止所有服務及緊急重啟（{_cmd}），發起者: {sid}")
...
@app.route('/api/llm_bridge/<agent>', methods=['POST','OPTIONS'])
def api_llm_bridge_proxy(agent):
    '''公網 LLM 橋接代理：直播房3 等 report 頁面經此轉發本機 127.0.0.1:8935（真 LLM），解決公網頁面 fetch 127.0.0.1 連不到的問題'''
    # ⚠️ 公網 agent 白名單：只准公開主播 id，防公網 relay 到 admin 型 agent（病毒引擎/凜…）
    _PUBLIC_LIVE_AGENTS = {"稚", "春", "影片女"}
    if agent not in _PUBLIC_LIVE_AGENTS:
        return jsonify({"text": "", "error": f"agent '{agent}' 唔喺公開主播白名單", "allowed": sorted(_PUBLIC_LIVE_AGENTS)}), 403
    ...
    try:
        ...
        body['agent'] = agent
        req = urllib.request.Request(
            'http://127.0.0.1:8935/?agent=' + urllib.parse.quote(agent),
            data=json.dumps(body).encode('utf-8'),
            headers={'Content-Type': 'application/json'})
        with urllib.request.urlopen(req, timeout=90) as r:
            raw = r.read()
        _resp = Response(raw, content_type='application/json; charset=utf-8')
        _resp.headers['Access-Control-Allow-Origin'] = '*'
        return _resp
    except Exception as e:
        print('[llm_bridge proxy] 錯誤:', e, flush=True)
        return jsonify({'text': '⚠ LLM 橋接暫時不可用：' + str(e)}), 502
...
def create_agent():
    ...
    # 統一線程數：新 Agent 一律 MOK_NUM_THREADS=4
    import re as _re
    if _re.search(r'(?m)^MOK_NUM_THREADS=', content):
        content = _re.sub(r'(?m)^MOK_NUM_THREADS=.*$', 'MOK_NUM_THREADS=4', content)
    else:
    ...
    try:
        with open(dot_file, 'r', encoding='utf-8') as _cf:
            for _line in _cf:
                ...
                elif _ls.startswith('MOK_Study='):
                    _study = _ls.split('=', 1)[1].strip()
    except Exception:
        pass
    _study = _study or '0200'
    _user_md = f"""202609080610
...
自主學習:
    每天 {_tz} 時區的 {_study} 妳可以自己搜尋 github 或其他網上 找提升
    {_post}
    能力的庫,可安裝在妳房間/jobs/yymmdd-hhmmss_工作目的/
    (立即設定定時功能:未設定)

自我意識:
    每次輸出最後,妳都應該總結經驗 寫入 .mok/agent/{name}/soul/EXP.md
...
所有工作:
    ...
    永遠一個工作項目相關的檔 必須放到同一資料夾中,

所有backups統一:
    /home/ubuntu/.mok/backups/_tmp_backups/


刪除任何檔案/目錄前，一律先執行：
    bash ~/.mok/tools/trash.sh <路徑...>
    → 項目會移入 ~/.mok/trash/（全系統回收站），保留 30 天後由 cron 自動清除。
    禁止直接物理刪除；trash/ 內 30 天內可依 .origin.log 反悔搬回。
    此規則由 Agent「備」於 2026-09-04 統一建立，屬全體共通守則。

嚴禁:
    重開 mok_agi， pm2 restart mok_agi，關閉 mok_agi
    (如要重啟 mok_agi 必須叫主人手操作)

===

## 一、頂層總覽 `/home/ubuntu/.mok/`

```
~/.mok/
├── core/                 # 核心引擎 Python（mokagi, tool_handler, config, logger…）
├── tools/                # 工具插件庫（每個 .py = 一個工具，可隨時新增）
├── frontends/            # 前端入口：mok_tg.py（Telegram）、mok_web.py + mok_web/（Web）
├── html/                 # Web 界面靜態檔（index.html、static、多個特化頁）
├── skill/                # 技能資料夾（每技能一資料夾 + README.md）
├── agent/                # 96 個 Agent 專屬目錄（見下）
├── scripts/              # 系統輔助腳本
├── backups/              # 備份區（tar.gz、db、log）
├── mpt/                  # MoneyPrinterTurbo 短影音工具與 materials
├── desktop_setup.log     # 桌面安裝日誌（自動只保留最近 1000 行）
├── work/                 # 工作文件暫存
├── browser_profiles/     # 瀏覽器持久化設定檔（多 profile 隔離登入）
├── .chroma_data/         # ChromaDB 向量資料庫（記憶 + 知識庫）
├── .memory/              # 記憶/歷史相關
├── .pending_admin_confirm/   # 待管理員確認指令
├── .pending_cron_confirm/    # 待確認 cron
├── .speech2text_models/  # 語音轉文字模型快取
├── trash/                # 資源回收桶（30 天後自動清除）
├── _tmp/                 # 暫存區（gui_agent 截圖 shots 等；7 天自動清理）
├── gui_agent -> _tmp/gui_agent   # 過渡 symlink：舊進程仍在用舊路徑，/reload 後可刪
├── ecosystem.config.js   # pm2 配置
├── env.env               # 環境變數
├── MOKAGI.sh             # 安裝/啟動腳本
├── README.md / _README.md
├── CPU_上傳.bat / CPU_備份.bat
└── （其他：playwright-browsers、whisper_models…）
```
"""
    _user_md = _user_md.replace('所有  工作', '所有 工作')
...
# ---------- 侍女 GPU 用量帳單（每侍女一個 DB：開機 / 預計關機 / 費用明細）----------
@app.route('/api/gpu/status')
def api_gpu_status():
    try:
        import gpu_billing
        data = gpu_billing.status_payload(sync_first=request.args.get('sync', '1') != '0')
        return jsonify(data)
    except Exception as e:
...
def api_gpu_sync():
    try:
        import gpu_billing
        return jsonify({"ok": True, "summary": gpu_billing.sync()})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
    ...
    with closing(sqlite3.connect(DB_PATH, timeout=30)) as conn:
        ...
        for row in rows:
            ...
            messages.append({
                ...
                "conv_id": row["conv_id"],
                "role": row["role"],
                "content": (row["content"] if (row["content"] or "").strip() or row["role"] != "assistant"
                            else ("（⚠️ 此輪僅記錄思考過程，回答未完整落盤）" if (row["think_content"] or "").strip()
                                  else "（⚠️ 此輪無文字回答）")),
...
def dynamic_page(page):
    ...
    if template.startswith('project/'):
        ...
        # 目錄請求（如 /project/、/project/MokCs/）→ 直接送 index.html，避免 Jinja2 誤解析
        _idx = os.path.join(BASE_DIR, page.rstrip('/'), 'index.html')
        if os.path.isfile(_idx):
            return send_file(_idx, conditional=True)
    try:
...
if __name__ == '__main__':
    ...
    # ===== Warden #3：優雅關機（SIGTERM/SIGINT） =====
    # 目的：重啟時不留下半死線程、不佔住 5000 端口，讓 warden/pm2 的 stop->start 乾淨俐落；
    #       用 exit 0 結束，launcher 就不會再印「意外退出（signal -15）」。
    import signal as _signal
    def _graceful_exit(signum, frame):
        try:
            print(f"[mok_web] 收到訊號 {signum}，優雅關機中…", flush=True)
            sys.stdout.flush()
            sys.stderr.flush()
        except Exception:
            pass
    ...
    try:
        ...
        print(f"[mok_web] signal handler 註冊失敗：{_e}", flush=True)
